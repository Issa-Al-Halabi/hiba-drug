...
*/
    ...
    public static function store_notif_to_db($customer_id, $data)
    {
        try {
            $data = json_encode($data, true);
            $notify = new OrderNotification;
            $notify->user_id = $customer_id;
            $notify->data = $data;
            $notify->save();
        } catch (Exception $e) {
        }
    ...
    public static function translateArabic($key)
    {
        $lang_array = include(base_path('resources/lang/sy/messages.php'));
        $processed_key = ucfirst(str_replace('_', ' ', Helpers::remove_invalid_charcaters($key)));

        if (!array_key_exists($key, $lang_array)) {
            $lang_array[$key] = $processed_key;
            $str = "<?php return " . var_export($lang_array, true) . ";";
            file_put_contents(base_path('resources/lang/sy/messages.php'), $str);
    ...
    public static function compareTwoQuantity($qtyNew, $qtyOld)
    {
        return $qtyOld - $qtyNew;
    }
}
